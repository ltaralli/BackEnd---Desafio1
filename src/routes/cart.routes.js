...
cartRouter.get('/:cid', async (req, res) => {
    ...
    if (!cart) {
      return res.status(404).send({ status: 'error', msg: 'El carrito no existe' });
    }
...
cartRouter.post('/', async (req, res) => {
    try {
        ...
        if (!cart) {
            return res.status(500).send({ status: 'error', msg: 'No se pudo crear el carrito' });
        }
...
cartRouter.post('/:cid/product/:pid', async (req, res) => {
    ...
    try {
        res.send({ status: 'successful', msg: 'Producto agregado al carrito correctamente'})
    } catch (error) {
        throw error
    }
})
